...
//! AliasMethod
//  \brief Implement the alias method to sample from a discrete probability
//  distribution
//
...
//  int* alias_table;
  ...
  explicit AliasMethod(const std::vector<double>& probabilities);

  // Sample from the discrete probability distribution defined by the
  // probabilities given to the constructor. The returned value is the
  // zero-based index of the sampled element. rand1 and rand2 are two
  // independent random variables drawn from the uniform distribution [0,1) that
  // need to be provided. This function can be called inside Kokkos loops on the
  // device.
  int Sample(double rand1, double rand2) const {
